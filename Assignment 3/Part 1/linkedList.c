...
static void removeLink(struct LinkedList* list, struct Link* link)
{
	...
	link->next->prev = temp->prev;
	link->prev->next = temp->next;
	free(temp);
	list->size--;
}
...
void linkedListPrint(struct LinkedList* deque)
{
	/* FIXME: You will write this function */
	assert(deque != 0);
	struct Link* linkPtr = deque->frontSentinel->next;
	if (linkedListIsEmpty(deque))
	{
		printf("The list was empty.\n");
		return;
	}
	printf("List from front to back is: ");
	while (linkPtr->next != 0)
	{
		printf(FORMAT_SPECIFIER " ", linkPtr->value);
