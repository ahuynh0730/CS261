...
* Email: huynhant@oregonstate.edu
...
TYPE listQueueFront(struct Queue* queue) 
{
   ...
   /* FIXME: You will write this function */
	assert(queue != 0);
	assert(listQueueIsEmpty(queue) != 0);
...
void listStackPush(struct Stack* stack, TYPE value) 
{
	...
	if (listQueueIsEmpty(stack->q1))
	{
		stack->q1->tail->next = newLink;
		if (!listQueueIsEmpty(stack->q2))
		{
			newLink->next = stack->q2->head->next;
			stack->q1->tail = stack->q2->tail;
			stack->q2->head->next = 0;
			stack->q2->tail = stack->q2->head;
		}
		else
		{
			stack->q1->tail = newLink;
		}
	}
	else
	{
		stack->q2->tail->next = newLink;
		if (!listQueueIsEmpty(stack->q1))
		{
			newLink->next = stack->q1->head->next;
			stack->q2->tail = stack->q1->tail;
			stack->q1->head->next = 0;
...
TYPE listStackPop(struct Stack* stack) 
{
	...
	int tempValue;
	//if (listQueueIsEmpty(stack->q1))
		//tempValue = listQueueRemoveFront(stack->q2);
	//else
...
TYPE listStackTop(struct Stack* stack) 
{
	...
	int tempValue;
	//if (listQueueIsEmpty(stack->q1))
		//tempValue = stack->q2->head->next->value;
	//else
		tempValue = stack->q1->head->next->value;
	return tempValue;
